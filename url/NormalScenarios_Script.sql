...
CREATE EXTENSION url; -- Creates the URL Extansion to use
Create table testurl(Emri url); -- Creates a new table for testing pourposes
CREATE INDEX testidx ON testurl(Emri); -- Creates index which we need to use to test the BTree index
...
Insert into testurl values(url('https','www.ulb.be','mycourses' ));--Creates a URL from the specified @NotNullOrEmpty protocol, host, and NotNull file

--###URL(URL context, varchar spec)
--context will be returned until <host> and what's after it, and they will be replaced 
...
Select to_string(url('https://www.ulb.be/servlet/search?page=&site=%23&l=1&RH=1538989254560&beanKey=150bfcee-1f87-11e7-a0e0-b753bedcad22&q=test#1'));
